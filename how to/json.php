...
<body>
    ...
    <div class = "container">
      <h1> Getting the juicy bits from json </h1> 
      ...
      <p> Let's dissect this code to understand what it does, because it's kind of confusing. The top line, "req.addeventlistener," basically is saying that we are waiting for event "load" and when that happens we will execute the function. This calls are all built into AJAX and javascript there are plenty more that you can also explore to get different information. Req.status basically asks for a time and if it is too high or low it will fail the program, this is helpful for if the server doesn't respond. The next line will log the json file you recieved onto the console on your browser (right-click on the webpage and press inspect), the one for Ivysaur will look like this: </p> </br>

      <img src="ivylook.png" height="300" width="1200"></br>
    </br>

      <p> This is really helpful for you because we can now see that the json file is categorized to make it easier to parse the information. However this information isn't really helpful to anyone but yourself as it is only visible in the console and you want your friends to see the real deal stuff so you can compare. This is done through the setting a variable (response in the image above) to the parsed json file so that way you can easily get the infromation you want. The next couple of lines labeled, "getElementById" are part of DOM, which is part of Javascript, you don't need to worry about that much about how it works directly because you probably are only going to use it to put information in your html. Which brings about the good point that "getElementById" searches the html that it is in to see if that id name exists and if it does it will then change in. I prefer to put my script in the header of the html document as to skip issues of improper loading. Remember that example I showed on the home page? Well here is the html for it: </p> </br> 

      <img src="pokehtml.png"></br></br>

      <p> As you can see I referenced those id names in my javascript so it changed it in the html to fit, notice when doing an image you need to reference ".src" instead of text content, this will cahnge the scr in the html img tag. Please know there are a ton of ways to reference documents in your html, <a href="http://www.w3schools.com/jsref/dom_obj_document.asp">here is a link</a> to w3schools which can explain more. </br>
      </br>
